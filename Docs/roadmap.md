...
Текущий baseline: **main 0.49.0**, build 146, 941 тест (10 пропущены). Следующее
исправление — 0.49.1; следующая пользовательская функция или новый источник — 0.50.0.
...
совпадает. После merge #213 все 54 открытых issues перечислены в этом разделе.
...
- **#219** — убрать из военного поиска регион и юридически невозможные категории
  апелляций на мировых судей; picker foundation #81 уже готов.
- **#211** — показывать номер производства текущей инстанции в заседаниях и
  ленте «Обзора» через существующий общий formatter.
- **#220** — синхронизировать регион и суд в обе стороны для областного,
  апелляционного и кассационного звена; #219 сначала фиксирует family-specific поля.
- **#110** — подробный отчёт CSV-import; атомарный batch и fallible commit уже готовы.
- Затем продолжается source gate **#88 + #164 + #89 + #207**. Зависимые задачи из
  карты разблокировок ниже вставляются сразу после закрытия соответствующего основания.

### 1. Correctness и source reliability до event journal

- **#88 + #164 + #89** продолжают source gate: CAPTCHA
  continuation/session и пригодное движение КСОЮ;
  между задачами нет искусственной жёсткой зависимости.
- **#207** — отдельный msudrf TLS failure ручного CAPTCHA-flow; исправление не
  должно отключать TLS verification и согласуется с session-контуром #164.
...
- **#56 → #70 → (#222 + #223 + #224) → (#129 + #125 + #111) → #128.** Сначала
  устраняется ложный срок при живой кассации. Затем rules engine закрепляет формулу,
  evidence, provenance и versioned predicates. На этой модели создаются отдельные
  исчерпывающие registry процессуальных сроков (#222), сроков рассмотрения (#223) и
  исторический LegalCalendar 2002+ (#224); только после них исправляются календарные
  месяцы, рабочие дни и перенос окончания без частных списков и эвристик.
...
| --- | --- |
| Уже выполнены #64, #79, #81, #82, #91, #117, #151, #210, #213 и identity/reconciliation foundation | Готовая очередь #219, #211, #220 и #110; #94 и #74 тоже архитектурно готовы, но стоят сразу после текущего source gate из-за большего scope |
| #88 + #164 | #92: добавление дела по URL использует единый надёжный CAPTCHA/session flow |
| #89 | #132 и production-aware lifecycle #90 + #130; отсутствие movement больше не скрывает источник номера и evidence активной карточки КСОЮ |
...
| #56 | #86: effective legal-force status использует тот же проверенный lifecycle source of truth |
| #70 | #222 + #223 + #224: versioned нормативные registry и LegalCalendar используют единые predicates, evidence и provenance |
...
| Стабильный source gate #88 + #164 + #89 и level-1 fixtures #181 | #106 → #108 → #107: московский adapter по проверенному reference, минимальный общий magistrate routing, затем источник СПб |

#94 уже разблокирована постоянной identity-моделью, а #74 — typed source outcomes и
консервативным cache merge. Они не ждут event journal, но идут после короткой готовой
очереди и текущего source gate из-за большего scope. #93 ждёт именно event contract,
