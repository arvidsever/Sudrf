...
Текущий baseline: **main 0.58.9**, build 179. Последняя зафиксированная полная проверка
приложения — в 0.58.8: 1328 XCTest (5 пропущены) и 28 Swift Testing; документационная
правка 0.58.9 не сопровождалась новым локальным запуском Swift-тестов или сборкой.
...
Сверка в ходе #291: 58 открытых issues на 12 сентября 2026 года, каждая учтена в очереди.
...
- **[#291](https://github.com/arvidsever/Sudrf/issues/291) + апелляционная часть [#76](https://github.com/arvidsever/Sudrf/issues/76)** — в работе: поиск КАС/УПК/ГПК из первой инстанции суда субъекта в АСОЮ и из окружного/флотского суда в Апелляционный военный суд. Подтверждены `3а-673/2026 → 66а-731/2026` и `2-12/2025 → 55-584/2025`; запрос пропускается из-за пустого набора апелляционных картотек. Проверить точные URL, УИД загруженной карточки, исключение материалов и штатное обновление старых записей. Военная живая пара ещё не подтверждена. Следующий кассационный маршрут остаётся в #76; #165 и #247 отдельно.
