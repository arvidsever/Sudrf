...
- **#181** проходит двумя уровнями. Уровень 1 готов: единый offline-harness
  ...
  (`old snapshot + new snapshot → CaseEvent[]`) остаётся заблокирован #155 и
  добавляется после стабилизации event vocabulary; до этого issue не закрывается.
