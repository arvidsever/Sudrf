...
Текущий baseline: **main 0.53.4**, build 154, 1035 тестов (5 пропущены). Следующая
...
- **#164** проходит в два доказательных шага. 0.53.2 локально накапливает только
  ...
  задача — общий lifecycle-блок **#90 + #130**.
...
- **#90 + #130**: production-aware lifecycle переводит реальную карточку КСОЮ по
  делу КоАП в отдельную стадию «Надзор» и активное кассационное звено.
...
| --- | --- |
| Уже выполнены #64, #79, #81, #82, #88, #89, #91, #110, #117, #132, #151, #207, #210, #211, #213, #219, #220 и identity/reconciliation foundation | #94 и #74 архитектурно готовы, но стоят после ближайшего слоя #90 + #130 из-за большего scope |
| #88 + #164 | #92: добавление дела по URL использует единый надёжный CAPTCHA/session flow |
| Выполненные #89 + #132 | production-aware lifecycle #90 + #130; отсутствие movement больше не скрывает источник номера и evidence активной карточки КСОЮ |
...
консервативным cache merge. Они не ждут event journal, но идут после общего lifecycle-
блока #90 + #130 из-за большего scope. #93 ждёт именно event contract,
