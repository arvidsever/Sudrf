...
/// перезапуск приложения ей незачем. Синглтон сохраняет одну пару для поиска,
/// фонового обновления и ручного продолжения CAPTCHA.
public actor CaptchaTokenStore {
    ...
    /// Ключ — дефисная (модульная) форма хоста: обе формы одного суда попадают
    /// в одну запись.
