...
Текущий baseline: **main 0.57.2**, build 163. Полный registry сроков обжалования
...
Сверено с GitHub 8 сентября 2026 года: **57 открытых issues**, все распределены
...
- **Восстановление ссылок после импорта** реализовано в `codex/import-card-recovery`
  (черновик 0.57.3), но ещё не смёржено. Только федеральный SUDRF: параметры
  картотеки, затем полный поиск в том же суде по УИД/номеру с проверкой карточки.
  Досье, подборки и успешный кэш сохраняются; временные ошибки не запускают замену.
  Профильные и полные тесты, сборка и три исходных live-примера проверены.
  Итоговая приёмка последней импортированной подборки с полным подсчётом результатов
  не завершена. Подробности — [черновик ветки](branch-changelogs/codex/import-card-recovery/v0.57.3.md).
  Это часть [#248](https://github.com/arvidsever/Sudrf/issues/248), а не завершение всего импорта. `MDB_MAP_FULL` исключён:
  причинная связь не подтверждена.

- **[#265](https://github.com/arvidsever/Sudrf/issues/265)** — реализовано в
  `codex/koap-complaint-metadata-265` (черновик 0.57.5): карточка жалобы без строки
  судебного УИД теряет опубликованные результат и дату рассмотрения, поэтому
  завершённое производство остаётся в «Надзоре». Подтверждены 16-3568/2021 и
  16-5035/2023; исправление ограничено чтением реквизитов и обычным обновлением.
  Полный набор: 1183 теста, 5 пропущены, 0 ошибок; Debug-сборка прошла.
  Обе карточки штатно обновлены и найдены в «Завершённых»; «Надзор»: 7 → 5.
  Issue остаётся открытой до merge. Перенос дат в строки движения — #256.
  Отдельно отмечено уменьшение общего счётчика при запуске (544 → 481),
  до обновления КоАП; оно требует отдельного сопоставления записей.
...
- **[#239](https://github.com/arvidsever/Sudrf/issues/239)** — реализовано в `codex/koap-primary-party-239`
  (черновик 0.57.4): основная сторона КоАП выбирается по роли привлекаемого лица,
  а не по порядку участников; общий formatter сохраняет защитника в полной карточке.
  Если основное лицо не опубликовано, сохраняется fallback с именем и явной ролью.
  Старые снимки без сети, полный набор тестов и Xcode-сборка проверены. Живая
  карточка вернула HTTP 403; issue остаётся открытым до merge и завершения приёмки.
