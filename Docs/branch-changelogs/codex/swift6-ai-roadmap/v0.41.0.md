...
- AI evidence теперь проверяется case-sensitive, критические реквизиты — с
  token boundaries в процитированных абзацах; добавлены словесные суммы и
  неполные даты. Предупреждения модели стали cited claims, локальные notices
  отделены от provider output.
- Translation spike валидирует английский результат до обратного перевода и
  требует точной последовательности placeholder IDs; подготовка языковой пары
  использует тот же shared actor, что и генерация.
- Spotlight по решению продукта включён по умолчанию с opt-out; отключение
  удаляет весь индекс Sudrf. Добавлен явный handler системного Spotlight.
- PDF App Intent переведён в foreground, `sudrf-cli` добавлен в обе CI-линии,
  HTTP error body больше не показывается пользователю.

## Проверка

- `swift build --product SudrfApp -Xswiftc -strict-concurrency=complete` — успешно.
- `swift test` — 485 тестов, успешно после добавления этапов 0–6; тот же полный
  набор успешно пройден Swift toolchain из Xcode 27 beta.
- `AISummaryPipelineTests` — 9 тестов: chunking/retry, translation literals,
  benchmark-пороги и HTTP-контракт Groq «только выбранный акт».
- Новые `ActDocumentTests`, `ActSummaryTests`, `DataCatalogTests`,
  `SpotlightIntegrationTests` и `AISummaryPipelineTests` — 30 тестов, успешно.
