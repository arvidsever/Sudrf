...
Текущий baseline: **main 0.47.2**, build 143, 913 тестов (5 пропущены). Следующее
исправление — 0.47.3; следующая пользовательская функция или новый источник — 0.48.0.
...
совпадает. После merge #117 все 49 открытых issues перечислены в этом разделе.
...
- **#210** — не позволять глобальному поиску перекрывать открытую карточку;
  независимый короткий UI-fix без архитектурных prerequisites.
- **#151** — явные ссылки на исходную карточку дела и каждой известной инстанции.
- **#213** — после #151 загружать и единообразно отображать опубликованные
  файловые акты DOC, DOCX и PDF, не принимая HTML/CAPTCHA за документ; после
  выполнения prerequisite задача поднимается в первый свободный слот.
...
| --- | --- |
| Уже выполнены #64, #79, #81, #82, #91, #117 и identity/reconciliation foundation | Готовая очередь #210, #151 → #213, #211 и #110; #94 и #74 тоже архитектурно готовы, но стоят сразу после текущего source gate из-за большего scope |
