...
Текущий baseline: **main 0.46.13**, build 140, 891 тест (5 пропущены). Следующее
исправление — 0.46.14; следующая пользовательская функция или новый источник — 0.47.0.
...
совпадает. После merge #82 все 49 открытых issues перечислены в этом разделе.
...
- **#91** — явное снятие дела с отслеживания: fallible persistence и защита от
  применения позднего refresh уже готовы.
- **#81** — корректная фильтрация гарнизонных и окружных военных судов: локальный
  picker-fix без архитектурных зависимостей.
- **#117** — не принимать страницу суда за судебный акт и не индексировать её.
...
| --- | --- |
| Уже выполнены #64, #79, #82 и identity/reconciliation foundation | Готовая очередь #91, #81, #117, #151, #110; #94 и #74 тоже архитектурно готовы, но стоят сразу после текущего source gate из-за большего scope |
