...
Текущий baseline: **main 0.58.15**, build 185. Для 0.58.15 выполнены профильные и полные
Swift-тесты, сборка Xcode 27 и сборка приложения. Сценарии четырёх дел проверены
изолированными интеграционными тестами.
...
Сверка 16 сентября 2026 года: после завершения #294 остаётся 56 открытых issues.
...
- **[#247](https://github.com/arvidsever/Sudrf/issues/247)** — в работе в `codex/material-production-context-247`: раздельные роль материала и контекст ГПК/КАС/УПК/КоАП. Собственные явные реквизиты и специализированная картотека, затем подтверждённая связь с основным делом (включая полный судебный УИД), затем индекс. Связь сильнее неоднозначного индекса; противоречие оставляет вид неизвестным. Старые материалы уточняются из кэша без объединения записей и без назначения общих сроков основного дела. PR [#303](https://github.com/arvidsever/Sudrf/pull/303): 1398 XCTest (5 пропусков), 28 Swift Testing и Xcode 27 build прошли. Живая приёмка ожидает резервной копии рабочей базы; main не изменён. Полный справочник v1.5 и новые правила сроков #222 в этот выпуск не входят.
- **[#246](https://github.com/arvidsever/Sudrf/issues/246)** — после доказанного перехода к принятому гражданскому делу выбирать актуальный номер и lifecycle, сохраняя прежнюю регистрацию. Общий механизм подтверждения, атомарного переякоривания и поиска прежних номеров выполнен для КАС в #284; гражданское расширение и приёмка, включая #288, остаются.
