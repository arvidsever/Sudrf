...
Текущий baseline: **0.58.34**, build 204. Полный локальный набор:
1517 XCTest (5 штатных пропусков) и 28 Swift Testing, без ошибок; Xcode Debug build.
...
Сверка 25 сентября 2026 года: после завершения #321 и постановки #329 открыты 45 issues;
...
- **[#329](https://github.com/arvidsever/Sudrf/issues/329)** — единая форма контролов: кнопки с текстом, переключатели, меню,
  чипы и поля ввода — капсулы, кнопки из одной иконки — круги. Макет одобрен автором
  25 сентября 2026 года. Системные segmented и всплывающие меню сначала проверяются
  на штатную капсулу; самописные замены — только при отсутствии системного пути.
  Косметика, patch-версия; обязательна визуальная приёмка.
