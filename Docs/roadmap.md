...
- **0.58.15 / #294:** активированы специальные пятидневные сроки апелляции и частной жалобы по явно указанным избирательным делам КАС. Для прекращения дела с обобщённой категорией применяется общий срок частной жалобы в 15 рабочих дней; участники и номер дела категорию не подменяют. Тестами покрыты сценарии `3а-682/683/684/685/2026`, обновление М→3а, ручная дата и единая проекция срока. PR [#302](https://github.com/arvidsever/Sudrf/pull/302).

- **0.58.14 / PR [#299](https://github.com/arvidsever/Sudrf/pull/299):** опубликован [справочник индексов v1.5](legal-indices/README.md) и военно-территориальный словарь ВС РФ. Исходники сохранены побайтово; проверены SHA-256, JSON и структура. Runtime не менялся; #247 и другие задачи внедрения остаются открытыми.

- **0.58.13 / #300:** все подтверждённые апелляции сохраняются в общей хронологии; последующее принятие первой инстанции определяет актуальную стадию и сроки. Цепочка 9а-77 → 66а-726 → 3а-681 и перезапуск проверены; значок ЧЖ удалён. PR [#301](https://github.com/arvidsever/Sudrf/pull/301).
...
- **0.58.12 / #291:** доменные алиасы и три карточки Самары подтверждены штатным обновлением и перезапуском; PR [#298](https://github.com/arvidsever/Sudrf/pull/298) влит. Полная хронология апелляций выполнена в #300.
