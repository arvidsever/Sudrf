...
- **[#247](https://github.com/arvidsever/Sudrf/issues/247)** — в работе в `codex/material-production-context-247`: раздельные роль материала и контекст ГПК/КАС/УПК/КоАП. Собственные явные реквизиты и специализированная картотека, затем подтверждённая связь с основным делом (включая полный судебный УИД), затем индекс. Связь сильнее неоднозначного индекса; противоречие оставляет вид неизвестным. Старые материалы уточняются из кэша без объединения записей и без назначения общих сроков основного дела. Профильные проверки проходят; полная и визуальная приёмка ещё не завершены. Полный справочник v1.5 и новые правила сроков #222 в этот выпуск не входят.
