...
public enum MosGorSudRouting {
    ...
    ///
    /// ПУСТОЕ множество = «раздел не определяется», фильтр не применяется
    /// (fail-open). Так бывает по двум причинам, и обе — не повод выдумывать
    /// путь:
    ///  • раздела на портале действительно НЕТ. Кассация (`instance = 4`) по
    ///    гражданским/уголовным/КАС даёт ключи `CN`/`UN`/`CN_KAS`, которых нет
    ///    ни в `mgsLinksMapping`, ни в `rsLinksMapping` самого портала:
    ///    кассации (КСОЮ, ВС) на mos-gorsud не публикуются, они живут на
    ///    sudrf.ru. На портале из «верхних» есть только КоАП-надзор
    ///    (`review-supervision`, ключ `AN`) — он замаплен и фильтруется;
    ///  • пересечение ключей уже́, чем реальность (напр. апелляции материалов
    ///    ст. 125 УПК лежат в общем разделе `appeal-criminal`).
    /// Поэтому пустое множество НЕ означает «показать нечего»: резать выдачу
    /// в ноль здесь нельзя, иначе получим ложное «ничего не найдено».
