...
Текущий baseline: **main 0.53.3**, build 153, 1030 тестов (5 пропущены). Следующая
пользовательская функция или новый источник — 0.54.0.
...
- **#164** проходит в два доказательных шага. 0.53.2 локально накапливает только
  ...
  независимого eligibility-gate. Пока корпус накапливается, следующая исполнимая
  задача — **#132**, затем общий lifecycle-блок **#90 + #130**.
...
- **#132**, затем **#90 + #130**: сначала закрепляется источник номера и evidence
  активной карточки КСОЮ, затем производство переводится на общий lifecycle-контур.
...
| --- | --- |
| Уже выполнены #64, #79, #81, #82, #88, #89, #91, #110, #117, #151, #207, #210, #211, #213, #219, #220 и identity/reconciliation foundation | #94 и #74 архитектурно готовы, но стоят после ближайшего слоя #132 и #90 + #130 из-за большего scope |
| #88 + #164 | #92: добавление дела по URL использует единый надёжный CAPTCHA/session flow |
| Выполненный #89 | #132 и production-aware lifecycle #90 + #130; отсутствие movement больше не скрывает источник номера и evidence активной карточки КСОЮ |
...
| Рабочие #88 + #89, завершённый #164 и level-1 fixtures #181 | #106 → #108 → #107: московский adapter по проверенному reference, минимальный общий magistrate routing, затем источник СПб |

#94 уже разблокирована постоянной identity-моделью, а #74 — typed source outcomes и
консервативным cache merge. Они не ждут event journal, но идут после #132 и общего
lifecycle-блока #90 + #130 из-за большего scope. #93 ждёт именно event contract,
