...
Текущий baseline: **main 0.58.31**, build 201. Полный локальный набор:
...
baseline на 200+ eligible-делах получен: обход 458/458 занял 13 ч 05 мин,
p95 ожидания в очереди — 12 ч 38 мин; starvation и дубли не обнаружены.
...
собственными HTML/PNG и больше не затрагивает отчёты обхода. Повторный
13-часовой замер для решения по #180 не требуется.
...
- **[#68](https://github.com/arvidsever/Sudrf/issues/68) + [#65](https://github.com/arvidsever/Sudrf/issues/65)** — диагностика источников и независимый live canary.
  ...
  и следующий естественный обход 458/458 сохранил baseline: 13 ч 05 мин, p95 ожидания
  12 ч 38 мин, без дублей и starvation. 437 пометок `vsrf.ru` не различают отказ,
  нераспознанную и подтверждённо пустую выдачу; причина длительности не доказана.
  Сначала уточняется контракт выдачи ВС РФ в **[#104](https://github.com/arvidsever/Sudrf/issues/104)**,
  затем **[#180](https://github.com/arvidsever/Sudrf/issues/180)** использует этот baseline для приоритетов
  и ограниченного backoff без повторного 13-часового замера. Host circuit breaker
