...
Текущий baseline: **main 0.57.1**, build 162, 1127 тестов (5 пропущены). Полный
...
- **Восстановление ссылок после импорта** — согласованный план реализуется в
  `codex/import-card-recovery` (черновик 0.57.3). Только федеральный SUDRF:
  проверка параметров картотеки, затем полный поиск в том же суде по УИД/номеру
  с проверкой самой карточки. Сохраняются досье, подборки и успешный кэш;
  временные ошибки не запускают замену. Приёмка: профильные и полные тесты,
  сборка, обычное обновление последней импортированной подборки с подсчётом
  результатов. Подробности — [черновик ветки](branch-changelogs/codex/import-card-recovery/v0.57.3.md).
