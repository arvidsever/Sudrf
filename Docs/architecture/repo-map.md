...
### Восстановление адреса карточки

`CaseCardRecovery` — общий неперсистентный механизм для импорта, repair-якоря
и обновления. Сначала читает исходную ссылку без изменений. Только после
подтверждённого отсутствия карточки или HTTP 404/410 проверяет параметры
подходящей картотеки, затем выполняет строгий поиск в исходном суде/экземпляре
базы по УИД и точному номеру. Неполная выдача, неоднозначность и противоречия
реквизитов запрещают замену; CAPTCHA и временные ошибки проходят обычным путём.

Проверенный адрес и source identity сохраняются через `TrackedStore` вместе
с историей существующего досье. Прежняя ссылка не должна возвращаться в активный
контекст при повторном импорте; последний успешный кэш и подборки сохраняются.
