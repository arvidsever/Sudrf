...
Текущий baseline: **main 0.54.0**, build 155, 1045 тестов (5 пропущены). Следующая
пользовательская функция или новый источник — 0.55.0.
...
- **#164** проходит в два доказательных шага. 0.53.2 локально накапливает только
  ...
  после завершённого lifecycle-блока #90 + #130 ближайшая очередь — **#94**, затем
  **#74**.
...
- **#94**: связать текущую карточку с опубликованным «номером по предыдущей
  регистрации», подтвердить continuity и сохранить хронологию последовательных
  регистраций без отдельного tracked-дубликата.
- **#74** (после #94): показывать под уже найденным связанным материалом его
  собственное движение через существующий source/network path, без ложного
  «движения нет» и без стирания последнего успешного результата.
...
| --- | --- |
| Уже выполнены #64, #79, #81, #82, #88, #89, #90, #91, #110, #117, #130, #132, #151, #207, #210, #211, #213, #219, #220 и identity/reconciliation foundation | #94, затем #74 — ближайшие готовые задачи |
| #88 + этап 1 #164 | #92: добавление дела по URL использует единый надёжный CAPTCHA/session flow |
| Выполненные #89 + #132 + #90 + #130 | #94, затем #74: история перерегистраций и движение уже найденных материалов |
...
консервативным cache merge. После выполненного lifecycle-блока #90 + #130 ближайшая
задача — #94, затем #74. #93 ждёт именно event contract,
