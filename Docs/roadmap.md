...
Текущий baseline: **main 0.53.1**, build 151, 993 теста (5 пропущены). Следующее
исправление — 0.53.2; следующая пользовательская функция или новый источник — 0.54.0.
...
совпадает. После merge #110 открытые issues перечислены в этом разделе.
...
- Следующий слой source gate — **#164**: автоматическое распознавание переиспользует
  уже рабочую CAPTCHA-сессию мировых судей из #207. **#88 + #89** остаются в gate;
  зависимые задачи из карты разблокировок ниже вставляются сразу после закрытия
  соответствующего основания.

### 1. Correctness и source reliability до event journal

- **#164** первой переиспользует session-flow #207 и подаёт в него результат
  локального распознавания вместо ручного кода, без второго сетевого решения.
- **#88 + #89** продолжают source gate: надёжная CAPTCHA/session continuation и
  пригодное движение КСОЮ; между ними нет искусственной жёсткой зависимости.
...
| --- | --- |
| Уже выполнены #64, #79, #81, #82, #91, #110, #117, #151, #207, #210, #211, #213, #219, #220 и identity/reconciliation foundation | #94 и #74 архитектурно готовы, но стоят сразу после текущего source gate из-за большего scope |
