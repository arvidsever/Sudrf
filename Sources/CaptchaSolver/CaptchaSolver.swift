...
public actor CaptchaSolver {
    ...
    /// Возвращает топ-N кандидатов от настроенного провайдера для
    /// диагностики. Применяет те же preprocess-правила, что и `solve`,
    /// если провайдер их поддерживает. Второй элемент кортежа — был ли
    /// применён preprocess.
    public func topCandidates(pngData: Data, kind: CaptchaKind, host: String? = nil, n: Int = 3) async -> (candidates: [(text: String, confidence: Double)], preprocessed: Bool) {
        do {
            return try await provider.topCandidates(pngData: pngData, kind: kind, host: host, n: n)
        } catch {
            return ([], false)
        }
    }
